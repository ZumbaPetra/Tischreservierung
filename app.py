...
def load_hotel_data():
  ...
  return {
      ...
      "Retz": {
          "email": "retz@austria-best-hotels.at",
          "max_gaeste": 40,
          "dauer_stunden": 2,
      },
  }
...
with st.form(key="reservation_form"):
  ...
  with col_uhrzeit:
    time_slots = [
        f"{h:02d}:{m:02d}" for h in range(7, 22) for m in (0, 15, 30, 45)
    ]
    uhrzeit = st.selectbox(
        "Uhrzeit", options=time_slots, index=6, disabled=is_storno
    )
  ...
  name = st.text_input(
      "Auf Name", placeholder="z.B. Max Mustermann", disabled=is_storno
  )

  col_tel, col_email = st.columns(2)
  with col_tel:
    telefon = st.text_input(
        "Telefonnummer", placeholder="z.B. +4366012345678", disabled=is_storno
    )
  with col_email:
    email = st.text_input(
        "E-Mail-Adresse", placeholder="z.B. gast@email.com", disabled=is_storno
    )
...
if submit_button:
  ...
  # B) VALIDIERUNG FÜR NEUE RESERVIERUNG
    ...
    # Hotel-E-Mail aus den geladenen Daten extrahieren
    ...
    payload = {
        "status": "Neu",
        "ort": selected_ort,
        "hotel_email": hotel_email,  # Wird an Google Apps Script übergeben
